...
import { Eye, EyeOff, Loader2, Zap, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authApi.register({ email: email.trim(), password, name: name.trim() });
          ...
          <CardContent className="p-5 pt-2">
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full h-10 bg-[#ff6363] hover:bg-[#f85353] text-white font-medium text-xs sm:text-sm mt-3 flex items-center justify-center"
              >
                {isLoading ? (
                  <>
